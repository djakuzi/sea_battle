...
@Injectable()
export class FriendRequestRepository extends StandartRepository<EntityFriendRequest> {
    constructor(
        private readonly dataSource: DataSource,
        @InjectRepository(EntityFriendRequest)
        private readonly repoFriendRequest: Repository<EntityFriendRequest>
    ) {
        super(dataSource, repoFriendRequest);
    }
    ...
    async createRequest(data: Partial<EntityFriendRequest>, manager?: EntityManager): Promise<EntityFriendRequest> {
    ...
    async deleteRequest(data: Partial<EntityFriendRequest>, manager?: EntityManager): Promise<DeleteResult> {
        const repo = this.getRepo(manager);

        return await repo.delete(data);
    }

    async findRequests(conditions: CustomOptionWhere<EntityFriendRequest>, manager?: EntityManager): Promise<EntityFriendRequest[] | null> {
        const repo = this.getRepo(manager);
    ...
    async findOneRequest(conditions: Partial<EntityFriendRequest>, manager?: EntityManager): Promise<EntityFriendRequest | null> {
        const repo = this.getRepo(manager);
    ...
    async findRequestByList(data: IntrFindRequestByList): Promise<EntityFriendRequest[] | null> {
        ...
        return await repo.createQueryBuilder('request')
            .where(
