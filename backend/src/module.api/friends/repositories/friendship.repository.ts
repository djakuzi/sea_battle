...
export class FriendShipRepository extends StandartRepository<EntityFriendship> {
    ...
    async createFriendship(player1Id: number, player2Id: number, manager?: EntityManager): Promise<EntityFriendship> {
        const repo = this.getRepo(manager);

        const [resPlayer1Id, resPlayer2Id] = [Math.min(player1Id, player2Id), Math.max(player1Id, player2Id)];
    ...
    async deleteFriendship(data: Partial<EntityFriendship>, manager?: EntityManager): Promise<DeleteResult> {
        const repo = this.getRepo(manager);
    ...
    async findFriendsByList(data: IntrFindFriendsByList): Promise<EntityFriendship[] | null> {
        ...
        const friendships = await repo.createQueryBuilder('friendship')
            .where(`
    ...
    async findFriendships(data: IntrFindFriendship): Promise<Partial<EntityPlayer>[]> {
        ...
        const qb = repo.createQueryBuilder('friendship')
            .leftJoin(EntityPlayer, 'friend',
                `(
