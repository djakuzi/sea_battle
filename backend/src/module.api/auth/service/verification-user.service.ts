...
export class ServiceVerififcationUser {
	...
	async findVerification(
		filter: Partial<EntityVerificationsUsers>,
		manager?: EntityManager
	): Promise<EntityVerificationsUsers | null> {
		const conditions = buildConditionsFindWhere<EntityVerificationsUsers, typeof filter>(
			filter,
			'AND'
		);
	...
	async addVerificationByUser(
		data: Partial<EntityVerificationsUsers>,
		manager?: EntityManager
	): Promise<EntityVerificationsUsers | null> {
		const res = await this.repoVerificationUser.createVerificationUser(data, manager);

		if (!res)
			throw new InternalServerErrorException('Произошла ошибка при создании верификации');
		return res;
	}
}
