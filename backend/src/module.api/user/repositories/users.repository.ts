...
export class UsersRepository extends StandartRepository<EntityUser> {
  ...
  async updateOne(idUser: number, data: Partial<EntityUser>, manager?: EntityManager): Promise<UpdateResult> {
    const repo = this.getRepo(manager);
    return await repo.update({ id: idUser }, data);
  }

  async findAll(conditions: CustomOptionWhere<EntityUser>, manager?: EntityManager): Promise<EntityUser[] | null> {
    const repo = this.getRepo(manager);
  ...
  async findOne(conditions: CustomOptionWhere<EntityUser>, manager?: EntityManager): Promise<EntityUser | null> {
    const repo = this.getRepo(manager);
