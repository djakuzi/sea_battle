...
export interface IntrSchemaStrategyMore extends IntrStandartSchemaStrategy<IntrArgsStrategyMore, TypeReturnStrategyMore> {
    args: IntrArgsStrategyMore,
    return: TypeReturnStrategyMore
}

@Injectable()
export class StrategyMore implements IntrStandartStrategy<EnumNameStrategyFindUser> {
    readonly name = EnumNameStrategyFindUser.MORE;
    ...
    async execute(args: IntrSchemaStrategyMore['args']): Promise<IntrSchemaStrategyMore['return']> {
        ...
        const user = await this.repoUsers.findAll(!conditions ? undefined : conditions, args.manager);
